...
//Funzione di lettura dello stato di un singolo dynamixel (posizione, velocità, coppia)
int * dxl_read_state( int id)
{
    while(giBusUsing);

    gbInstructionPacket[ID] = (unsigned char)id;
    gbInstructionPacket[INSTRUCTION] = INST_READ;
    gbInstructionPacket[PARAMETER] = (unsigned char)36; //Start from present_position (36)
    gbInstructionPacket[PARAMETER+1] = 6;
    gbInstructionPacket[LENGTH] = 4;

    dxl_txrx_packet();

    static int r[3];
    r[0] = dxl_makeword((int)gbStatusPacket[PARAMETER], (int)gbStatusPacket[PARAMETER+1]);
    r[1] = dxl_makeword((int)gbStatusPacket[PARAMETER+2], (int)gbStatusPacket[PARAMETER+3]);
    r[2] = dxl_makeword((int)gbStatusPacket[PARAMETER+4], (int)gbStatusPacket[PARAMETER+5]);
